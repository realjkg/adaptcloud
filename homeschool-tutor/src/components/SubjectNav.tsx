...
export default function SubjectNav({ subjects, currentSubject, completed, onNext, disabled }: Props) {
  ...
  return (
    <nav className="bg-white rounded-xl border border-navy-100 shadow-sm p-4">
          ...
          return (
            <div
              ...
              className={`flex items-center gap-3 min-h-[48px] rounded-lg px-3 py-2 transition-all ${
                isCurrent
                  ? 'border-l-4 border-navy-500 pl-2 bg-navy-50'
                  : isDone
            ...
            >
              ...
              <div className="flex-shrink-0">
                {isDone ? (
                  ...
                  <div className="w-4 h-4 rounded-full bg-navy-500 flex items-center justify-center">
                    <svg
                      width="10"
                      height="10"
                      viewBox="0 0 10 10"
              ...
              {/* Subject icon */}
              <info.Icon size={16} className="flex-shrink-0 text-current" />

              {/* Label + duration */}
              <div className="flex-1 min-w-0">
                <div
                  className={`text-sm font-medium truncate ${
                    isCurrent ? 'text-navy-800' : 'text-gray-600'
                  }`}
                >
                  {info.label}
                </div>
      ...
      {hasNext && (
        <button
          onClick={onNext}
          disabled={disabled}
          className="mt-4 w-full min-h-[44px] flex items-center justify-center gap-2 rounded-lg text-sm font-medium bg-navy-500 text-white hover:bg-navy-600 disabled:opacity-40 disabled:cursor-not-allowed transition-colors px-4"
        >
          Next Subject
