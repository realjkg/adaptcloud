(async()=>{try{
  ...
  const note=document.getElementById("blueprintNote");if(note){
    if(Object.keys(profile.domains||{}).length)note.textContent=`${profile.code} domain weights are isolated from every other certification. Adaptive practice targets the weakest weighted domains first; full mocks activate only when the unique bank can satisfy the complete blueprint.`;
